...
Begin
  b:=False;
  kol_same:=0;
//  if(num=3)and(n=9)then
//        writeln('тройка чекается');
//  if(num=3)and(n=9)then
//  begin
//    writeln('TUT');
//    writeln('Цвет: ', id);
//    writeln('Количесвто возможных мест: ',kol_same);
//    writeln('Последняя пара: ',l[1],'-',l[2]);
//  end;  
  for i:=1 to n do
  begin
    for j:=1 to n do
...
//      writeln(var_kletka[8,3],' ERROR [',8,',',3,'] 1');  
//      writeln('[',1,',',7,'] :: ',var_kletka[1,7],' 1');  
      end;
      if x = 0 then
          break;
      // для столбца
...
//      writeln(var_kletka[8,3],' ERROR [',8,',',3,'] 2');    
//      writeln('[',1,',',7,'] :: ',var_kletka[1,7],' 2');  
      end;
      if x = 0 then
          break;
      // для блока
...
//    writeln(var_kletka[8,3],' ERROR [',8,',',3,'] 3');  
//    writeln('[',1,',',7,'] :: ',var_kletka[1,7],' 3');  
...
//    writeln(var_kletka[8,3],' ERROR [',8,',',3,'] 4'); 
//    writeln('[',1,',',7,'] :: ',var_kletka[1,7],' 4');   
    if(x=n)and(ositaa>0)then 
    begin
    // голые пары
    // в СТОЛБЦЕ
...
//                  writeln(var_kletka[HSame,j],' FOR DELETE. i=',Ha[HI],'+',HSame,' j=',j);
//                  writeln(arr[Hsame,j]);
//                  writeln(var_kletka[NITAV,j],' ---->','  PROVERIT. i=',NITAV,' j=',j);
                  if HDel>0 then
                  begin
                    delete(var_kletka[NITAV,j],HDel,1);
                    x:=0;
                  end;
//                  writeln(var_kletka[NITAV,j],' <----');
...
//      writeln(var_kletka[8,3],' ERROR [',8,',',3,'] 5(1)');
//      writeln('[',1,',',7,'] :: ',var_kletka[1,7],' 5(1)');  
    end;   
    if(x=n)and(ositaa>0)then 
    begin
    // голые пары
...
//                  writeln(var_kletka[i,HSame],' FOR DELETE. i=',i,' j=',Ha[Hi],'+',HSame);
//                  writeln(arr[i,Hsame]);
//                  writeln(var_kletka[i,NITAV],' ---->','  PROVERIT. i=',i,' j=',NITAV,' :: ',var_kletka[i,Ha[Hi]],' and ',var_kletka[i,HSame]);
                  if HDel>0 then
                  begin
                    delete(var_kletka[i,NITAV],HDel,1);
                    x:=0;
                  end;
//                  writeln(var_kletka[i,NITAV],' <----');
...
//      writeln(var_kletka[8,3],' ERROR [',8,',',3,'] 5(2)');  
//        writeln('[',1,',',7,'] :: ',var_kletka[1,7],' 5(2)');
    end;
   if(x=n)and(ositaa>0)then
    for i:=1 to n do
    begin
      ...
      begin
        ...
        begin
          ...
          writeln('[',i,',',j,'] ГОЛАЯ: ',var_kletka[i,j],' --- ', lake_grid[i,j]);
        end;
        if length(var_kletka[i,j])=1 then
        begin
          writeln('[',i,',',j,'] ЛИШЬ ОДНО МЕСТО: ',var_kletka[i,j],' --- ', k);
          x:=0;
          dec(ositaa);
          num:=strtoint(var_kletka[i,j]);
          arr[i,j]:=num;
      ...
      if x>0 then
        x:=CloseAlone(x);
//      writeln('[',1,',',7,'] :: ',var_kletka[1,7],' Одиночка');  
      if x>0 then
        x:=Hidetwink(x);
//      writeln('[',1,',',7,'] :: ',var_kletka[1,7],' скрытая пара');  
    end;
    x:=wow_skin_box(x);
    inc(x);
  end;
...
//  writeln(var_kletka[8,3],' ERROR [',8,',',3,']');
//    writeln('[',1,',',7,'] :: ',var_kletka[1,7]);    
  closefile(f);
  auto_solve;
  ...
  begin 
    for j:=1 to n do
    begin
      if(arr[i,j]=0)and(var_kletka[i,j]='')then
        writeln('ERROR [',i,',',j,']');
//      if(arr[i,j]=0)then
//        writeln('FUCK THAT [',i,',',j,'] :: ',var_kletka[i,j]);
      if(arr[i,j]=0)and(length(var_kletka[i,j])=1)then
        writeln('Не воспользовались:)');
