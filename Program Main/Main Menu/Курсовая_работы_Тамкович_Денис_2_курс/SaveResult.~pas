...
// Unit заполнения данных о выигравшем пользователе

// Появляется после того, как Игрок решил головоломку
// и согласился сохранить свои результаты
unit SaveResult;
...
type
  TForm6 = class(TForm)
    ...
    procedure BitBtn1Click(Sender: TObject); // кнопка Сохранить (для сохранения результатов)
    procedure BitBtn2Click(Sender: TObject); // кнопка Отмена (для отмены сохранения результатов)
    procedure Edit1KeyPress(Sender: TObject; var Key: Char); // отслеживание вводимых символов
  private
